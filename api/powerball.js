...
module.exports = async (req, res) => {
  try {
    const response = await fetch('https://www.powerball.com/previous-results');
    ...
    $('table tbody tr').each((i, el) => {
      ...
      if (tds.length >= 7) {
        ...
        const powerball = parseInt($(tds[6]).text().trim(), 10);
        results.push({ date, numbers, powerball });
      }
    });
    res.status(200).json({ draws: results });
  } catch (err) {
    res.status(500).json({ error: 'Failed to fetch Powerball data', details: err.message });
  }
}; 